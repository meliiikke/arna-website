...
const NewsInsightDetail = () => {
  ...
  const fetchArticle = async () => {
    try {
      ...
      // Add a minimum delay to prevent flickering on fast connections
        const response = await apiGet(`/api/news-insight-details/${id}`);
      ...
      if (response && !response.error) {
        ...
        setArticle({
          id: id,
          title: 'Haber Detayı',
          description: 'Bu haber hakkında detaylı bilgi için lütfen bizimle iletişime geçin.',
          image_url: null,
          wysiwyg_content: '',
          content_sections: []
        });
  ...
  if (loading) {
    return (
      <div className="news-insight-detail-page">
        <Header />
        <div className="news-insight-detail-container">
  ...
  const renderContentSections = () => {
    if (!article.content_sections) return null;
    
    let contentSections = [];
    try {
      const parsed = typeof article.content_sections === 'string' 
        ? JSON.parse(article.content_sections) 
  ...
  return (
    <div className="news-insight-detail-page smooth-enter">
      <Header />
      
      <div className="news-insight-detail-container">
        {/* Back Button */}
        <div className="news-insight-detail-header">
          <button onClick={goBack} className="btn-back">
            ← Geri Dön
          </button>
        </div>
        ...
        {/* Article Content */}
        <div className="news-insight-detail-content">
          <h1 className="news-insight-detail-title">{article.title}</h1>
          
          {/* WYSIWYG İçerik */}
          {article.wysiwyg_content ? (
            <div 
              className="wysiwyg-content-display"
              dangerouslySetInnerHTML={{ __html: article.wysiwyg_content }}
            />
          ) : (
            ...
            <div className="content-sections-fallback">
              ...
              {(!article.wysiwyg_content && (!article.content_sections || JSON.parse(article.content_sections || '[]').length === 0)) && (
...
export default NewsInsightDetail;
